...
set udp_port_mastah 47858
set udp_port_servq 55646
...
proc parseServInfo { token } {
  ...
  if { $cplayers > 32 } {
	#puts "[clock seconds] DBG: banned from $dbg - skipping"
  } else {
...
proc requestNext {sock lastip0 lastip1 lastip2 lastip3 port} {
 ...
 # bytecode for ypsilon - country filter / all
  ...
  #
  if {$port == 0} {
    puts -nonewline $sock "1${a}0.0.0.0:0${c}${b}gamedir${b}NeotokyoSource${b}napp${b}500"
    ...
    puts -nonewline $sock "1${a}${lastip0}.${lastip1}.${lastip2}.${lastip3}:${port}${c}${b}gamedir${b}NeotokyoSource${b}napp${b}500"
 ...
 global stats_servers sock_mastah retrycount 
  if { $stats_servers == 0 } {
    if { $retrycount == 0 } {
      ...
      udp_conf $sock_mastah "68.142.72.250" "27011"
      requestNext $sock_mastah 0 0 0 0 0
      #debug
      #puts "[clock seconds] retrying on 68.142.72.250:27011"  
    } elseif { $retrycount == 3 } {
      set retrycount 0
      udp_conf $sock_mastah "69.28.140.247" "27011"
      requestNext $sock_mastah 0 0 0 0 0
      #debug
...
#submitStats
printToCache "ntplayers.cache"

#update rrd
updateRrdfile "./nt_rrd/ntstats.rrd"
