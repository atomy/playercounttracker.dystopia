...
set udp_port_mastah 47889
set udp_port_servq 55667
...
proc parseServers { servers } {
  ...
  for { set i 1 } { $i < [llength $servs] } { incr i } {
    ...
    # debug
    puts "got server: $i: $ip0.$ip1.$ip2.$ip3:$port"
 #   puts "$i: $test2"
  }
...
proc parseServInfo { token } {
  ...
  #
#  puts "DBG: $dbg"
  puts "DBG: cplayers: $cplayers -- maxplayers: $maxplayers -- bots: $bots -- os: $os -- vac: $vac -- passw: $passw"
}
...
proc requestNext {sock lastip0 lastip1 lastip2 lastip3 port} {
 ...
 # bytecode for ypsilon - country filter / all
  ...
  #
  if {$port == 0} {
    puts -nonewline $sock "1${a}0.0.0.0:0${c}{b}noplayers${b}1"

    # Debug stuff
    #uts -nonewline $sock "1${a}0.0.0.0:0${c}${b}gamedir${b}dystopia_v1${b}empty${b}1"
    ...
    puts -nonewline $sock "1${a}${lastip0}.${lastip1}.${lastip2}.${lastip3}:${port}${c}${b}noplayers${b}1"
...
after 20000 set state plzexit

# sock for querying valve mastah serv
#udp_conf $sock_mastah "hl2master.steampowered.com" "27011"
udp_conf $sock_mastah "72.165.61.189" "27011"
#udp_conf $sock_mastah "69.28.140.246" "27011"
#udp_conf $sock_mastah "68.142.72.250" "27010"
...
#submitStats
#printToCache "dysplayers.cache"

#update rrd
#updateRrdfile "./rrd/dysstats.rrd"
