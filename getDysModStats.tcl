...
set udp_port_mastah 47890
set udp_port_servq 55670
...
proc requestNext {sock lastip0 lastip1 lastip2 lastip3 port} {
 ...
 # bytecode for ypsilon - country filter / all
  ...
  #
  if {$port == 0} {
    puts -nonewline $sock "1${a}0.0.0.0:0${c}${b}gamedir${b}dystopia_v1${b}napp${b}50"
    ...
    puts -nonewline $sock "1${a}${lastip0}.${lastip1}.${lastip2}.${lastip3}:${port}${c}${b}gamedir${b}dystopia_v1${b}napp${b}50"
...
proc retry { } {
 global stats_servers sock_mastah
  if { $stats_servers == 0 } {
    requestNext $sock_mastah 0 0 0 0 0
    #puts "[clock seconds] retrying..." 
  }
}
